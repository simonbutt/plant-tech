...
def _set_log_level(log_env: str) -> None:
    ...
    log_level = (
        "INFO"
        if (env_result := log_env) not in approved_log_levels
        else env_result
    )
...
if __name__ == "__main__":
    ...
    DL: Type[DataLoader] = DataLoader(volume_path=data_path)
    dataset: fo.Dataset = (
        DL._demo_dataset() 
        if is_demo.lower() == "true" else 
        DL.load_data(config=dataset_config)
    )
    ...
    session: fo.Session = fo.launch_app(
        dataset,
        remote=True,
        port=5151
    )
